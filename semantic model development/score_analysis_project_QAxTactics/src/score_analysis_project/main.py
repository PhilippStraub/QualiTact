...
def main():
    ...
    qualitaetsattribute_df = pd.read_excel(BASE_DIR / "data" / version / "data-quality-attributes.xlsx")
    taktiken_df = pd.read_excel(BASE_DIR / "data" / version / "data-tactics.xlsx")

    # Check whether line numbers are the same
    assert len(qualitaetsattribute_df) == len(taktiken_df), 'ERROR | Number of lines in tactics and QA files differs – alignment not possible.'

    # Drop NaN-values
    qualitaetsattribute_df = qualitaetsattribute_df.apply(pd.to_numeric, errors='coerce').fillna(0).astype(int)
    taktiken_df = taktiken_df.apply(pd.to_numeric, errors='coerce').fillna(0).astype(int)

    # Drop weak columns
    qualitaetsattribute_df = drop_weak_columns(qualitaetsattribute_df, "Qualitätsattribute")
    taktiken_df = drop_weak_columns(taktiken_df, "Taktiken")

    # compute metrics for all qa-tactic pairs
    results = []
    qa_cols = list(qualitaetsattribute_df.columns)
    tactic_cols = list(taktiken_df.columns)

    for qa, tactic in product(qa_cols, tactic_cols):
        m = compute_metrics(qualitaetsattribute_df[qa], taktiken_df[tactic])
        if m['n_series_1'] < MIN_SUPPORT_QA:
            continue
        if m['n_series_2'] < MIN_SUPPORT_TACTIC:
            continue
        m.update({'QA': qa, 'Tactic': tactic})
        results.append(m)
    ...
    norm_df['w_p'] = norm_df.apply(compute_w_p, axis=1)
    ...
    for index, row in norm_df.iterrows():
        ...
        sum1 = int(qualitaetsattribute_df[qa].sum())
        sum2 = int(taktiken_df[tac].sum())

        json_output["Scores"].append({
            "QualityAttribute": qa,
            "QualityAttribute_Sum": sum1,
            "Tactic": tac,
            "Tactic_Sum": sum2,
    ...
    csv_output_df.rename(columns={'Score': 'Score_default'}, inplace=True) # Rename for consistency with notebook
    final_cols = [
        'QA', 'Tactic', 'a', 'b', 'c', 'd', 'n', 
        'n_series_1', 'n_series_2', 'confidence', 
        'logRR', 'phi', 'p_value', 'w_p', 'logRR_norm', 'phi_norm', 
